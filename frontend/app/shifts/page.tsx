...
import { MapPin, Clock, Coins, Filter } from 'lucide-react';
...
export default function ShiftsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-4">{t('shifts.browseShifts')}</h1>
        
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <div className="flex items-center mb-4">
            <Filter className="w-5 h-5 mr-2 rtl:mr-0 rtl:ml-2 text-gray-600" />
            <h2 className="text-lg font-semibold">{t('shifts.filters')}</h2>
          </div>
          <div className="grid md:grid-cols-4 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{t('shifts.industry')}</label>
              <select
                className="input-field bg-white text-gray-900"
                value={filters.industry}
                onChange={(e) => setFilters({ ...filters, industry: e.target.value })}
              >
                <option value="">{t('shifts.allIndustries')}</option>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{t('shifts.city')}</label>
              <input
                type="text"
                className="input-field bg-white text-gray-900"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{t('shifts.minWage')}</label>
              <input
                type="number"
                className="input-field bg-white text-gray-900"
                placeholder="500000"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{t('shifts.maxWage')}</label>
              <input
                type="number"
                className="input-field bg-white text-gray-900"
                placeholder="1000000"
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-4 rtl:space-x-reverse">
                    ...
                    <span className="text-gray-600 text-sm">
                      {language === 'fa' 
                        ? `${formatPersianNumber(shift.filled_positions || 0)} / ${formatPersianNumber(shift.number_of_positions)} ${t('shifts.positionsFilled')}`
                        : `${shift.filled_positions || 0} / ${shift.number_of_positions} ${t('shifts.positionsFilled')}`}
                    </span>
                  </div>
                </div>
